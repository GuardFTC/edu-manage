...
func Test_hyperLogLogClient(t *testing.T) {
	...
	//2.运行测试
	t.Run("redis hyperloglog客户端测试", func(t *testing.T) {
		h := HyperLogLogClient

		//1.添加单个元素到HyperLogLog
		count, err := h.PFAdd("hll_key", "element1")
		if count != 1 || err != nil {
			t.Error(err)
		}

		//2.添加多个元素到HyperLogLog
		count, err = h.PFAdd("hll_key", "element2", "element3", "element4")
		if count != 1 || err != nil {
			t.Error(err)
		}

		//3.添加重复元素到HyperLogLog
		count, err = h.PFAdd("hll_key", "element1", "element2", "element5")
		if count != 1 || err != nil {
			t.Error(err)
		}

		//4.获取HyperLogLog的基数估算值
		card, err := h.PFCount("hll_key")
		if card != 5 || err != nil {
			t.Error("PFCount结果不符合预期")
		}

		//5.创建另一个HyperLogLog
		count, err = h.PFAdd("hll_key2", "element4", "element5", "element6", "element7")
		if count != 1 || err != nil {
			t.Error(err)
		}

		//6.获取第二个HyperLogLog的基数估算值
		card, err = h.PFCount("hll_key2")
		if card != 4 || err != nil {
			t.Error("PFCount结果不符合预期")
		}

		//7.获取多个HyperLogLog的合并基数估算值
		card, err = h.PFCount("hll_key", "hll_key2")
		if card != 7 || err != nil {
			t.Error("多个HyperLogLog的PFCount结果不符合预期")
		}

		//8.合并多个HyperLogLog
		err = h.PFMerge("hll_merged", "hll_key", "hll_key2")
		if err != nil {
			t.Error(err)
		}

		//9.获取合并后HyperLogLog的基数估算值
		card, err = h.PFCount("hll_merged")
		if card != 7 || err != nil {
			t.Error("合并后PFCount结果不符合预期")
		}

		//10.添加新元素到合并后的HyperLogLog
		count, err = h.PFAdd("hll_merged", "element8", "element9")
		if count != 1 || err != nil {
			t.Error(err)
		}
		...
		card, err = h.PFCount("nonexistent_hll")
		if card != 0 || err != nil {
			t.Error("不存在的HyperLogLog的PFCount结果不符合预期")
		}

		//13.合并包含不存在的HyperLogLog
		err = h.PFMerge("hll_merged2", "hll_key", "nonexistent_hll")
		if err != nil {
			t.Error(err)
		}

		//14.获取包含不存在HyperLogLog的合并结果
		card, err = h.PFCount("hll_merged2")
		if card != 5 || err != nil {
			t.Error("包含不存在HyperLogLog的合并结果不符合预期")
		}

		//15.测试大量元素
		testBulkHyperLogLog(t, h, "hll_bulk", 1000)

		//16.清理测试数据
		cleanupKeys(t, []string{"hll_key", "hll_key2", "hll_merged", "hll_merged2", "hll_bulk"})
	})
}

// 测试大量元素的HyperLogLog
func testBulkHyperLogLog(t *testing.T, h *hyperLogLogClient, key string, count int) {
...
func cleanupKeys(t *testing.T, keys []string) {
	...
	for _, key := range keys {
		exists, err := StringClient.Exists(key)
		if exists != 0 || err != nil {
