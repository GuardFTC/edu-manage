...
func Test_setClient(t *testing.T) {
	...
	t.Run("redis set客户端测试", func(t *testing.T) {
		...
		//1.添加单个元素
		count, err := s.SAdd("set_key", "value1")
		if count != 1 || err != nil {
			t.Error(err)
		}

		//2.添加多个元素
		count, err = s.SAdd("set_key", "value2", "value3", "value4")
		if count != 3 || err != nil {
			t.Error(err)
		}

		//3.添加重复元素
		count, err = s.SAdd("set_key", "value1", "value2", "value5")
		if count != 1 || err != nil {
			t.Error(err)
		}

		//4.验证集合元素数量
		card, err := s.SCard("set_key")
		if card != 5 || err != nil {
			t.Error("SCard结果不符合预期")
		}

		//5.验证元素是否存在
		exists, err := s.SIsMember("set_key", "value1")
		if !exists || err != nil {
			t.Error("SIsMember结果不符合预期")
		}

		//6.验证元素不存在
		exists, err = s.SIsMember("set_key", "nonexistent")
		if exists || err != nil {
			t.Error("SIsMember不存在元素结果不符合预期")
		}

		//7.获取所有元素
		members, err := s.SMembers("set_key")
		...
		//8.随机获取一个元素
		randMembers, err := s.SRandMember("set_key")
		...
		//9.随机获取多个元素
		randMembers, err = s.SRandMember("set_key", 3)
		...
		//12.随机弹出多个元素
		popMembers, err = s.SPop("set_key", 2)
		...
		//14.移除元素
		count, err = s.SRem("set_key", "nonexistent")
		if count != 0 || err != nil {
		...
		//16.移除存在的元素
		count, err = s.SRem("set_key", members[0])
		if count != 1 || err != nil {
			t.Error("SRem结果不符合预期")
		}
		...
		//18.准备集合操作测试
		_, err = s.SAdd("set_key1", "a", "b", "c", "d")
		if err != nil {
			t.Error(err)
		}
		_, err = s.SAdd("set_key2", "c", "d", "e", "f")
		if err != nil {
			t.Error(err)
		}

		//19.测试集合交集
		inter, err := s.SInter("set_key1", "set_key2")
		if err != nil {
			t.Error(err)
		}
		sort.Strings(inter)
		...
		//20.测试集合交集并存储
		count, err = s.SInterStore("set_inter", "set_key1", "set_key2")
		if count != 2 || err != nil {
			t.Error("SInterStore结果不符合预期")
		}
		inter, err = s.SMembers("set_inter")
		if err != nil {
			t.Error(err)
		}
		sort.Strings(inter)
		...
		//21.测试集合并集
		union, err := s.SUnion("set_key1", "set_key2")
		if err != nil {
			t.Error(err)
		}
		sort.Strings(union)
		...
		//22.测试集合并集并存储
		count, err = s.SUnionStore("set_union", "set_key1", "set_key2")
		if count != 6 || err != nil {
			t.Error("SUnionStore结果不符合预期")
		}
		union, err = s.SMembers("set_union")
		if err != nil {
			t.Error(err)
		}
		sort.Strings(union)
		...
		//23.测试集合差集
		diff, err := s.SDiff("set_key1", "set_key2")
		if err != nil {
			t.Error(err)
		}
		sort.Strings(diff)
		...
		//24.测试集合差集并存储
		count, err = s.SDiffStore("set_diff", "set_key1", "set_key2")
		if count != 2 || err != nil {
			t.Error("SDiffStore结果不符合预期")
		}
		diff, err = s.SMembers("set_diff")
		if err != nil {
			t.Error(err)
		}
		sort.Strings(diff)
		...
		//25.测试元素移动
		_, err = s.SAdd("set_source", "item1", "item2")
		if err != nil {
			t.Error(err)
		}
		_, err = s.SAdd("set_dest", "item3")
		if err != nil {
			t.Error(err)
		}

		//26.移动元素
		moved, err := s.SMove("set_source", "set_dest", "item1")
		if !moved || err != nil {
			t.Error("SMove结果不符合预期")
		}

		//27.验证源集合
		members, err = s.SMembers("set_source")
		if len(members) != 1 || members[0] != "item2" || err != nil {
			t.Error("SMove后源集合结果不符合预期")
		}

		//28.验证目标集合
		members, err = s.SMembers("set_dest")
		if err != nil {
			t.Error(err)
		}
		...
		//29.移动不存在的元素
		moved, err = s.SMove("set_source", "set_dest", "nonexistent")
		if moved || err != nil {
			t.Error("SMove不存在元素结果不符合预期")
		}

		//30.清理测试数据
		cleanupSets(t, s, []string{"set_key", "set_key1", "set_key2", "set_inter", "set_union", "set_diff", "set_source", "set_dest"})

		//31.测试空集合的各种操作
		testEmptySetOperations(t, s, "empty_set")
	})
}
...
// 清理测试集合
func cleanupSets(t *testing.T, s *setClient, keys []string) {
	...
	for _, key := range keys {
		...
		//3.如果集合不为空，则删除所有元素
		if len(members) > 0 {
			_, err = s.SRem(key, interfaceSlice(members)...)
			if err != nil {
				t.Error(err)
			}
		}
...
// 测试空集合的各种操作
func testEmptySetOperations(t *testing.T, s *setClient, key string) {
