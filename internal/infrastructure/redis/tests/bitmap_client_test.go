...
func Test_bitMapClient(t *testing.T) {
	...
	t.Run("redis bitmap客户端测试", func(t *testing.T) {
		...
		//1.1 用户在1月1日签到
		val, err := b.SetBit(ctx, "user:sign:1:202501", 0, 1)
		if err != nil || val != 0 {
			t.Error(err)
		}

		//1.2 用户在1月3日签到
		val, err = b.SetBit(ctx, "user:sign:1:202501", 2, 1)
		if err != nil || val != 0 {
			t.Error(err)
		}

		//1.3 用户在1月5日签到
		val, err = b.SetBit(ctx, "user:sign:1:202501", 4, 1)
		if err != nil || val != 0 {
			t.Error(err)
		}

		//1.4 检查用户1月1日是否签到
		val, err = b.GetBit(ctx, "user:sign:1:202501", 0)
		if err != nil || val != 1 {
			t.Error("GetBit结果不符合预期，用户1月1日应该已签到")
		}

		//1.5 检查用户1月2日是否签到
		val, err = b.GetBit(ctx, "user:sign:1:202501", 1)
		if err != nil || val != 0 {
			t.Error("GetBit结果不符合预期，用户1月2日应该未签到")
		}

		//1.6 检查用户1月3日是否签到
		val, err = b.GetBit(ctx, "user:sign:1:202501", 2)
		if err != nil || val != 1 {
			t.Error("GetBit结果不符合预期，用户1月3日应该已签到")
		}

		//1.7 统计用户1月份签到次数
		count, err := b.BitCount(ctx, "user:sign:1:202501", &redisv9.BitCount{Start: 0, End: -1})
		if err != nil || count != 3 {
			t.Errorf("BitCount结果不符合预期，用户1月份应签到3次，实际为%d次", count)
		}

		//1.8 查找用户1月份第一次签到日期
		pos, err := b.BitPos(ctx, "user:sign:1:202501", 1, 0, -1)
		if err != nil || pos != 0 {
			t.Errorf("BitPos结果不符合预期，用户1月份第一次签到应为1月1日，实际为1月%d日", pos+1)
		}

		//1.9 查找用户1月份第一次未签到日期
		pos, err = b.BitPos(ctx, "user:sign:1:202501", 0, 0, -1)
		if err != nil || pos != 1 {
		...
		_, err = b.SetBit(ctx, "role:permissions:admin", 0, 1) // 读取权限
		if err != nil {
			t.Error(err)
		}
		_, err = b.SetBit(ctx, "role:permissions:admin", 1, 1) // 写入权限
		if err != nil {
			t.Error(err)
		}
		_, err = b.SetBit(ctx, "role:permissions:admin", 2, 1) // 删除权限
		if err != nil {
			t.Error(err)
		}

		//2.2 设置普通用户角色权限(只有读取权限)
		_, err = b.SetBit(ctx, "role:permissions:user", 0, 1) // 读取权限
		if err != nil {
			t.Error(err)
		}

		//2.3 检查管理员是否有读取权限
		val, err = b.GetBit(ctx, "role:permissions:admin", 0)
		if err != nil || val != 1 {
			t.Error("GetBit结果不符合预期，管理员应该有读取权限")
		}

		//2.4 检查管理员是否有删除权限
		val, err = b.GetBit(ctx, "role:permissions:admin", 2)
		if err != nil || val != 1 {
			t.Error("GetBit结果不符合预期，管理员应该有删除权限")
		}

		//2.5 检查普通用户是否有读取权限
		val, err = b.GetBit(ctx, "role:permissions:user", 0)
		if err != nil || val != 1 {
			t.Error("GetBit结果不符合预期，普通用户应该有读取权限")
		}

		//2.6 检查普通用户是否有写入权限
		val, err = b.GetBit(ctx, "role:permissions:user", 1)
		if err != nil || val != 0 {
			t.Error("GetBit结果不符合预期，普通用户应该没有写入权限")
		}

		//2.7 计算管理员拥有的权限数量
		count, err = b.BitCount(ctx, "role:permissions:admin", &redisv9.BitCount{Start: 0, End: -1})
		if err != nil || count != 3 {
			t.Errorf("BitCount结果不符合预期，管理员应有3个权限，实际为%d个", count)
		}

		//2.8 计算普通用户拥有的权限数量
		count, err = b.BitCount(ctx, "role:permissions:user", &redisv9.BitCount{Start: 0, End: -1})
		if err != nil || count != 1 {
		...
		len, err := b.BitOpOr(ctx, "role:permissions:combined", "role:permissions:admin", "role:permissions:user")
		if err != nil || len < 1 {
			t.Error(err)
		}

		//3.2 检查合并后的权限
		val, err = b.GetBit(ctx, "role:permissions:combined", 0)
		if err != nil || val != 1 {
			t.Error("BitOpOR结果不符合预期，合并后应该有读取权限")
		}

		val, err = b.GetBit(ctx, "role:permissions:combined", 1)
		if err != nil || val != 1 {
			t.Error("BitOpOR结果不符合预期，合并后应该有写入权限")
		}

		val, err = b.GetBit(ctx, "role:permissions:combined", 2)
		if err != nil || val != 1 {
			t.Error("BitOpOR结果不符合预期，合并后应该有删除权限")
		}

		//3.3 计算两个角色的权限交集(同时拥有的权限)
		len, err = b.BitOpAnd(ctx, "role:permissions:common", "role:permissions:admin", "role:permissions:user")
		if err != nil || len < 1 {
			t.Error(err)
		}

		//3.4 检查交集权限
		val, err = b.GetBit(ctx, "role:permissions:common", 0)
		if err != nil || val != 1 {
			t.Error("BitOpAND结果不符合预期，共同权限应该包含读取权限")
		}

		val, err = b.GetBit(ctx, "role:permissions:common", 1)
		if err != nil || val != 0 {
			t.Error("BitOpAND结果不符合预期，共同权限不应该包含写入权限")
		}

		//3.5 计算管理员独有的权限(管理员有但普通用户没有的权限)
		len, err = b.BitOpXor(ctx, "role:permissions:unique", "role:permissions:admin", "role:permissions:user")
		if err != nil || len < 1 {
			t.Error(err)
		}

		//3.6 检查独有权限
		val, err = b.GetBit(ctx, "role:permissions:unique", 0)
		if err != nil || val != 0 {
			t.Error("BitOpXOR结果不符合预期，独有权限不应该包含读取权限")
		}

		val, err = b.GetBit(ctx, "role:permissions:unique", 1)
		if err != nil || val != 1 {
			t.Error("BitOpXOR结果不符合预期，独有权限应该包含写入权限")
		}

		val, err = b.GetBit(ctx, "role:permissions:unique", 2)
		if err != nil || val != 1 {
			t.Error("BitOpXOR结果不符合预期，独有权限应该包含删除权限")
		}

		//3.7 计算权限的补集(取反)
		len, err = b.BitOpNot(ctx, "role:permissions:not_user", "role:permissions:user")
		if err != nil || len < 1 {
			t.Error(err)
		}

		//3.8 检查补集权限
		val, err = b.GetBit(ctx, "role:permissions:not_user", 0)
		if err != nil || val != 0 {
			t.Error("BitOpNOT结果不符合预期，普通用户权限的补集不应该包含读取权限")
		}

		val, err = b.GetBit(ctx, "role:permissions:not_user", 1)
		if err != nil || val != 1 {
		...
		//4.测试边界情况
		//4.1 测试大偏移量(模拟稀疏数据)
		_, err = b.SetBit(ctx, "sparse:bitmap", 10000, 1)
		if err != nil {
			t.Error(err)
		}

		val, err = b.GetBit(ctx, "sparse:bitmap", 10000)
		if err != nil || val != 1 {
			t.Error("大偏移量GetBit结果不符合预期")
		}

		//4.2 测试不存在的key
		val, err = b.GetBit(ctx, "nonexistent:bitmap", 0)
		if err != nil || val != 0 {
			t.Error("不存在的key GetBit结果不符合预期")
		}

		//5.模拟在线用户统计场景
		testOnlineUserStatistics(t, b, ctx)

		//6.清理测试数据
		cleanupBitmapKeys(t, ctx, []string{
			"user:sign:1:202501",
...
// 测试在线用户统计场景
func testOnlineUserStatistics(t *testing.T, b *bitmappkg.Client, ctx context.Context) {
	...
	for i := 0; i < 100; i += 2 {
		_, err := b.SetBit(ctx, key, int64(i), 1)
		if err != nil {
			t.Error("记录用户在线状态失败")
			return
		}
	...
	//3.统计在线用户数量
	onlineCount, err := b.BitCount(ctx, key, &redisv9.BitCount{Start: 0, End: -1})
	if err != nil {
		t.Error(err)
		return
	}
	...
	for i := 0; i < 10; i++ {
		userID := int64(i)
		val, err := b.GetBit(ctx, key, userID)
		if err != nil {
			t.Error(err)
			continue
		}
	...
	//6.查找第一个在线用户
	firstOnline, err := b.BitPos(ctx, key, 1, 0, -1)
